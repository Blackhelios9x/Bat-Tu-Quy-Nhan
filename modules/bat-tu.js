...
body::before { content: ''; position: fixed; inset: 0; background: rgba(255,250,240,.3); z-index: 0; pointer-events: none; }
.wrap { position: relative; z-index: 1; max-width: 1000px; margin: 0 auto; padding: 20px 16px 50px; }
...
<div class="wrap">
  ...
  <div class="tab-bar">
    <button class="tab-btn active" id="tb-dt" onclick="switchTab('dt')">🔥 Dụng Thần</button>
    <button class="tab-btn" id="tb-qn" onclick="switchTab('qn')">✨ Quý Nhân</button>
  </div>

  <!-- TAB DỤNG THẦN -->
  <div class="tab-panel active" id="panel-dt">
    <div class="card">
    ...
    <button class="btn-main" onclick="calcDungThan()">🔥 Tìm Dụng Thần</button>
  ...
  <!-- TAB QUÝ NHÂN -->
  <div class="tab-panel" id="panel-qn">
    ...
    <button class="btn-main" onclick="calcQuyNhan()">✨ Xem Quý Nhân</button>
...
let uploadedSuggestions = null;
let currentBazi = null;
let currentDaiVanList = null;
let currentDungThanInfo = null;

/* ========== HELPER ========== */
function nhCss(nh){ return nh>=0 ? NH_CSS[nh] : ''; }
function trungHoacSinh(nhA, nhB){ return nhB===nhA || SINH[nhB]===nhA; }

function getBaZi(y,m,d,h,mi){
  const solar = Solar.fromYmdHms(y,m,d,h,mi,0);
  const ec = solar.getLunar().getEightChar();
  return { year:parseGZ(ec.getYear()), month:parseGZ(ec.getMonth()), day:parseGZ(ec.getDay()), hour:parseGZ(ec.getTime()) };
}
function parseGZ(gz){
  const si=CAN_HAN.indexOf(gz[0]), bi=CHI_HAN.indexOf(gz[1]);
  return { si,bi, can:si>=0?CAN[si]:'?', chi:bi>=0?CHI[bi]:'?', nhCan:si>=0?NH_CAN[si]:-1, nhChi:bi>=0?NH_CHI[bi]:-1 };
}
...
function getSuggestionContent(hanhIndex) {
  const hanhName = NH_NAME[hanhIndex];
  if (uploadedSuggestions && uploadedSuggestions.has(hanhName)) {
    return uploadedSuggestions.get(hanhName);
  }
...
/* ========== ĐẠI VẬN ========== */
function tinhDaiVan(bz, birthY, birthM, birthD, isMale) {
  ...
  for (let n = 1; n <= 8; n++) {
    ...
    dvList.push({ si, bi, can: CAN[si], chi: CHI[bi], nhCan: NH_CAN[si], nhChi: NH_CHI[bi], year: dvYear, age: dvAge });
  }
  return { dvList, isThuan, targetJie, diffDays, khoiVanAge, firstDVYear, firstDVAge };
}

function renderDaiVan(dvData) {
  if (!dvData) {
    document.getElementById('dt-daivan-section').style.display = 'none';
    return;
  }
  const { dvList, isThuan, diffDays, khoiVanAge, firstDVYear, firstDVAge, targetJie } = dvData;
  const titleEl = document.querySelector('#dt-daivan-section .dv-title');
  ...
  dvList.forEach((dv, idx) => {
    gridHtml += `<div class="dv-item" onclick="showDaiVanDetail(${idx})">
      <div class="dv-age">${dv.age} tuổi<span class="dv-year">${dv.year}</span></div>
      <div class="dv-canchi nh-${nhCss(dv.nhCan)}">${dv.can}</div>
      <div class="dv-canchi nh-${nhCss(dv.nhChi)}">${dv.chi}</div>
    </div>`;
  });
  document.getElementById('dt-daivan-grid').innerHTML = gridHtml;
  document.getElementById('dt-daivan-section').style.display = 'block';
  document.getElementById('dv-detail-box').style.display = 'none';
...
function generateDaiVanDescription(bz, dv, info) {
  ...
  function xung(i1,i2){ return LUC_XUNG.some(p=>(p[0]===i1&&p[1]===i2)||(p[1]===i1&&p[0]===i2)); }
  function hại(i1,i2){ return LUC_HAI.some(p=>(p[0]===i1&&p[1]===i2)||(p[1]===i1&&p[0]===i2)); }
  function hợp(i1,i2){ return LUC_HOP.some(p=>(p[0]===i1&&p[1]===i2)||(p[1]===i1&&p[0]===i2)); }
  ...
  ['year','month','day','hour'].forEach(t => {
    ...
    else if (hại(dv.bi, c)) desc += `- Hại chi ${TRULBL[t]} → hao tổn<br>`;
    else if (hợp(dv.bi, c)) desc += `- Hợp chi ${TRULBL[t]} → hòa hợp<br>`;
  });
...
function renderLuuNien(bz) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    html += `<div class="ln-item" onclick="showLuuNienDetail(${year})">
      <div class="ln-year">${year}</div>
      <div class="ln-canchi nh-${nhCss(nhGan)}">${gan}</div>
      <div class="ln-canchi nh-${nhCss(nhChi)}">${chi}</div>
    </div>`;
  }
  document.getElementById('dt-luunien-grid').innerHTML = html;
  document.getElementById('dt-luunien-section').style.display = 'block';
  document.getElementById('ln-detail-box').style.display = 'none';
}

function getCurrentDaiVan(year) {
  if (!currentDaiVanList) return null;
  for (let i = currentDaiVanList.length - 1; i >= 0; i--) {
    if (year >= currentDaiVanList[i].year) return currentDaiVanList[i];
  }
  return currentDaiVanList[0];
}

function showLuuNienDetail(year) {
  if (!currentBazi || !currentDungThanInfo) return;
  const solar = Solar.fromYmdHms(year, 1, 1, 0, 0, 0);
  const lunar = solar.getLunar();
  const ganIdx = lunar.getYearGanIndex();
  ...
  const gan = CAN[ganIdx], chi = CHI[zhiIdx];
  const nhGan = NH_CAN[ganIdx], nhChi = NH_CHI[zhiIdx];
  const currentDV = getCurrentDaiVan(year);
  const { bz, dt1, dt2, isWeak, nhCan } = currentDungThanInfo;
  const desc = generateLuuNienDescription(bz, { year, gan, chi, ganIdx, zhiIdx }, { dt1, dt2, isWeak, nhCan }, currentDV);
  document.getElementById('ln-detail-title').innerHTML = `✨ Luận giải năm ${year} (${gan} ${chi})`;
  document.getElementById('ln-detail-content').innerHTML = desc;
...
function generateLuuNienDescription(bz, yearObj, info, dv) {
  ...
  truKeys.forEach((k, idx) => {
    ...
    else if (hại(zhiIdx, cChi)) desc += `- Hại chi ${truNames[idx]} → hao tổn, thị phi.<br>`;
    else if (hợp(zhiIdx, cChi)) desc += `- Hợp chi ${truNames[idx]} → hòa hợp, tin vui.<br>`;
  });
...
function calcDungThan(){
  ...
  const dateStr = document.getElementById('dt-date').value.trim();
  const parsed = parseDateDMY(dateStr);
  if (!parsed) { document.getElementById('dt-err').textContent = '⚠ Ngày không hợp lệ (dd/mm/yyyy)'; return; }
  const { d, m, y } = parsed;
  const h = Number(document.getElementById('dt-hour').value);
  const isMale = document.getElementById('dt-gender').value === 'male';

  let bz;
  try { bz = getBaZi(y, m, d, h, 0); }
  catch(e) { document.getElementById('dt-err').textContent = 'Lỗi: '+e; return; }
  ...
  currentBazi = bz;
  currentDungThanInfo = { bz, dt1, dt2, isWeak, nhCan: nhA };
  ...
  const content1 = getSuggestionContent(dt1);
  const content2 = getSuggestionContent(dt2);
  verdictEl.innerHTML = `
    <div class="verdict-title">${title}</div>
    <div class="verdict-body">${desc}</div>
    <div class="nh-tags">
      <div class="nh-tag ${nhCss(dt1)}">❶ Dụng Thần chính · ${NH_NAME[dt1]}</div>
      <div class="nh-tag ${nhCss(dt2)}">❷ Dụng Thần phụ · ${NH_NAME[dt2]}</div>
    </div>
    <div class="suggest-box">
      <div class="suggest-title">✨ Gợi ý chi tiết ${uploadedSuggestions ? '(từ GitHub)' : '(mặc định)'}</div>
  ...
  setTimeout(()=> document.getElementById('dt-result').scrollIntoView({behavior:'smooth',block:'start'}), 80);
}

function renderBaZiTable(bz){
  ...
  COLS.forEach(col=>{
    ...
    html+=`<div class="bz-col"><span class="bz-col-lbl">${col.lbl}</span>
      <div class="bz-cell nh-${nhCss(t.nhCan)}">${t.can}</div>
      <div class="bz-cell nh-${nhCss(t.nhChi)}">${t.chi}</div></div>`;
  });
  document.getElementById('dt-bz-table').innerHTML=html;
...
function calcQuyNhan(){
  ...
  const myDate = parseDateDMY(document.getElementById('myDate').value);
  const myTime = parseTime(document.getElementById('myTime').value);
  let hasErr = false;
  if(!myDate){ document.getElementById('myDate').classList.add('invalid'); document.getElementById('myErr').textContent='⚠ Ngày không hợp lệ'; hasErr=true; }
  if(!myTime){ document.getElementById('myTime').classList.add('invalid'); document.getElementById('myErr').textContent+=' ⚠ Giờ không hợp lệ'; hasErr=true; }
  if(hasErr){ document.getElementById('qn-results').style.display='none'; return; }

  let myBZ;
  try{ myBZ = getBaZi(myDate.y, myDate.m, myDate.d, myTime.h, myTime.min); }
  catch(e){ document.getElementById('myErr').textContent='Lỗi: '+e.message; return; }
  ...
  for(let i=1;i<=5;i++){
    ...
    if(!dRaw) continue;
    const pd = parseDateDMY(dRaw);
    if(!pd){ document.getElementById(`p${i}d`).classList.add('invalid'); document.getElementById(`p${i}e`).textContent='⚠ Ngày không hợp lệ'; continue; }
    const pt = parseTime(tRaw);
    if(tRaw && !pt){ document.getElementById(`p${i}t`).classList.add('invalid'); document.getElementById(`p${i}e`).textContent='⚠ Giờ không hợp lệ'; continue; }
    let pBZ;
    try{ pBZ = getBaZi(pd.y, pd.m, pd.d, pt?pt.h:0, pt?pt.min:0); }
  ...
  let html = `
    <div class="my-card">
      ...
      <div class="bz-row">${renderPillarsQN(myBZ)}</div>
      <div style="color:var(--muted);margin-top:4px">${baziStrQN(myBZ)}</div>
  ...
  if(people.length===0){
    ...
    people.forEach(p=>{
      const cmp = compareQN(myBZ, p.bz);
      html += `
        <div class="pcard">
          <div class="pcard-hd"><span class="badge">Người ${p.idx}</span> ${baziStrQN(p.bz)}</div>
          <div class="bz-row">${renderPillarsQN(p.bz, cmp.bHit, cmp.aHit)}</div>
          <div class="cmp-grid">
            ${renderPctQN(cmp.aPercent,cmp.aCount,'🌟 Quý nhân của <strong>bạn</strong><br>đối với người này',cmp.aHit)}
            ${renderPctQN(cmp.bPercent,cmp.bCount,'🌟 Quý nhân của người này<br>đối với <strong>bạn</strong>',cmp.bHit)}
          </div>
  ...
  document.getElementById('qn-results').innerHTML = html;
  document.getElementById('qn-results').style.display = 'block';
  setTimeout(()=> document.getElementById('qn-results').scrollIntoView({behavior:'smooth',block:'start'}), 80);
}

function compareQN(myBZ, pBZ){
  const aHit=[], bHit=[];
  ORDER_QN.forEach(p=>{
...
function renderPillarsQN(bz, hitA=[], hitB=[]){
...
function baziStrQN(bz){ return ORDER_QN.map(p=>`${bz[p].can} ${bz[p].chi}`).join(', '); }
function renderPctQN(pct, count, label, hits){
...
window.addEventListener('DOMContentLoaded', ()=>{
  const cont = document.getElementById('pInputs');
  for(let i=1;i<=5;i++){
    cont.innerHTML += `
