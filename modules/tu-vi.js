// ========================================================
// MODULE TỬ VI – LÁ SỐ & LUẬN GIẢI (Web Component)
// ========================================================
class TuViModule extends HTMLElement {
  constructor() {
    super();
    // Toàn bộ HTML & CSS của giao diện Tử Vi (không dùng Shadow DOM để giữ tương thích ID)
    this.innerHTML = /* html */ `
      <style>
        /* ========== CSS RIÊNG CHO TỬ VI ========== */
        #inputScreen {
        ...
        .btn-luangiai:hover { background: #8f613f; transform: translateY(-2px); }
        .btn-luangiai:active { transform: translateY(4px); box-shadow: 0 2px 0 #4a2e1e; }
      ...
      <div id="resultScreen">
        ...
        <div style="text-align: center; margin-top: 30px;">
          <button id="btnLuanGiai" class="btn-luangiai">📜 Luận giải lá số</button>
        </div>
        <div id="luanGiaiOutput" style="display: none; margin-top: 30px;"></div>
      </div>
    `;
  }
  ...
  connectedCallback() {
    if (typeof Solar === 'undefined') {
      alert('Lỗi: Không tải được thư viện lịch.');
      return;
    }

    // Khởi tạo giá trị mặc định và sự kiện
    this.querySelector('#namXem').value = new Date().getFullYear();
    this.querySelector('#btnXemLaSo').addEventListener('click', () => {
      this._updateLaSo();
      this._showResult();
    });
    this.querySelector('#backBtn').addEventListener('click', () => this._showInput());
    this.querySelector('#btnLuanGiai').addEventListener('click', () => this._thucHienLuanGiai());

    // Tải quy tắc luận giải
    this._fetchRules();
  }

  // ──────────────────────────────────────────────
  // HẰNG SỐ TĨNH (Static getters)
  // ──────────────────────────────────────────────
  ...
  static get posToChi() {
    return {
      1:'Tỵ',2:'Ngọ',3:'Mùi',4:'Thân',5:'Dậu',6:'Tuất',7:'Hợi',8:'Tý',9:'Sửu',10:'Dần',11:'Mão',12:'Thìn'
    };
  }
  static get chiToPos() {
    const map = {};
    for (let p in TuViModule.posToChi) map[TuViModule.posToChi[p]] = parseInt(p);
    return map;
  }
  static get STAR_PRIORITY() {
    return [
      'Hóa Lộc','Hóa Quyền','Hóa Khoa','Hóa Kỵ',
      'Thiên Không','Địa Kiếp','Kình Dương','Đà La','Linh Tinh','Hỏa Tinh',
      'Văn Xương','Văn Khúc','Tả Phù','Hữu Bật','Lộc Tồn',
  ...
  _computeQuocAnDuongPhu(locTonStartPos) {
    return {
      quocAn: this._forwardSteps(locTonStartPos, 7),
      duongPhu: this._backwardSteps(locTonStartPos, 6)
    };
  }
  ...
  _updateLaSo() {
    ...
    if (loaiLich === 'duong') {
      ...
      else { alert('Ngày âm không hợp lệ.'); return; }
    }
    ...
    // Bổ sung vị trí Hóa Khoa / Hóa Kỵ nếu chưa xác định
  ...
  // ──────────────────────────────────────────────
  _showInput() { this.querySelector('#inputScreen').style.display = 'block'; this.querySelector('#resultScreen').style.display = 'none'; }
  _showResult() { this.querySelector('#inputScreen').style.display = 'none'; this.querySelector('#resultScreen').style.display = 'block'; }
  ...
  async _fetchRules() {
    try {
      const resp = await fetch('https://raw.githubusercontent.com/Blackhelios9x/Bat-Tu-Quy-Nhan/main/luangiai.txt');
      if (!resp.ok) throw new Error();
      const text = await resp.text();
      this._loadedRules = this._parseRules(text);
      console.log(`✅ Đã nạp ${this._loadedRules.length} quy tắc luận giải.`);
    } catch (err) {
      console.warn('⚠️ Không thể tải quy tắc luận giải từ GitHub:', err);
      this._loadedRules = [];
    }
  }
  ...
  _parseRules(text) {
    ...
    for (let line of lines) {
      line = line.trim();
      if (!line) { if (cur?.condition_raw) { rules.push(cur); cur = null; } continue; }
      if (line.includes('|')) {
        const parts = line.split('|').map(p => p.trim());
        if (parts.length >= 3) {
          if (cur?.condition_raw) rules.push(cur);
          rules.push({ type: parts[0] || 'CHUNG', condition_raw: parts[1], loi_giai: parts[2], tho: parts[3] || '' });
          cur = null; continue;
        }
      }
      const hasCondition = /^(sao|hạn|cách|bộ|lưu|tiểu)\s/i.test(line) ||
        (/\b(gặp|hội|đồng cung|tam hợp|chiếu|giáp)\b/i.test(line) && /[A-ZÀ-ỹ][a-zà-ỹ]+(?:\s+[A-ZÀ-ỹ][a-zà-ỹ]+)*/.test(line));
      if (hasCondition && !cur) {
        cur = { type: '', condition_raw: line, loi_giai: '', tho: '' };
      } else if (cur) {
        if (/^[""“+*]/.test(line)) {
          cur.tho += (cur.tho ? '\n' : '') + line.replace(/^[""“+*]\s*/, '').replace(/["”’'’]$/, '');
        } else {
          cur.loi_giai += (cur.loi_giai ? ' ' : '') + line;
        }
      }
    }
    if (cur?.condition_raw) rules.push(cur);

    const DIA_BAN_LIST = ['Tý','Sửu','Dần','Mão','Thìn','Tỵ','Ngọ','Mùi','Thân','Dậu','Tuất','Hợi'];
    const KEYWORD_SKIP = new Set(['Hạn','Cách','Sao','Bộ','Hội','Gặp','Tại','Hoặc','Thủ','Vô','Hãm','Miếu','Hợp']);

    return rules.map(r => {
      let type = r.type;
      if (!type) {
        if (/hạn/i.test(r.condition_raw)) type = 'HẠN';
        else if (/cách|cục/i.test(r.condition_raw)) type = 'CÁCH';
        else type = 'SAO';
      }
      const rawTokens = [];
      const regex = /[A-ZÀ-ỹ][a-zà-ỹ]+(?:\s+[A-ZÀ-ỹ][a-zà-ỹ]+)*/g;
      let m;
      while ((m = regex.exec(r.condition_raw)) !== null) rawTokens.push(m[0].trim());
      const stars = rawTokens.filter(t => !DIA_BAN_LIST.includes(t) && !KEYWORD_SKIP.has(t)).map(t => t.toUpperCase());
      let relation = 'co_mat';
      const cr = r.condition_raw;
      if (cr.includes('đồng cung')) relation = 'dong_cung';
      else if (cr.includes('tam hợp')) relation = 'tam_hop';
      else if (cr.includes('đối cung') || cr.includes('xung chiếu')) relation = 'doi_cung';
      else if (cr.includes('nhị hợp')) relation = 'nhi_hop';
      else if (cr.includes('giáp')) relation = 'giap';
      else if (cr.includes('hội') || cr.includes('gặp')) relation = 'hoi_hop';
      return { type, condition_raw: r.condition_raw, stars, relation, loi_giai: r.loi_giai, tho: r.tho };
    });
  }

  _thucHienLuanGiai() {
    const laso = this._extractLaSo();
    if (!laso) return alert('⚠️ Chưa có dữ liệu lá số.');
    if (!this._loadedRules?.length) return alert('⚠️ Chưa tải được quy tắc luận giải.');

    let gocIdx = -1;
    for (let i = 1; i <= 12; i++) {
      const tenEl = this.querySelector(`#cung${i}Ten`);
      if (tenEl && tenEl.innerText.trim().toUpperCase() === 'MỆNH') {
        const chiEl = this.querySelector(`#cung${i}Chi`);
        const chiText = chiEl ? chiEl.innerText.trim() : '';
        const chi = chiText.split(/\s+/).pop();
        gocIdx = TuViModule.CHI.indexOf(chi);
        break;
      }
    }
    if (gocIdx === -1) gocIdx = 0;

    const results = [];
    for (const rule of this._loadedRules) {
      const match = this._matchRule(rule, laso, gocIdx);
      if (match) results.push(match);
    }
    results.sort((a, b) => b.matchScore - a.matchScore);

    const container = this.querySelector('#luanGiaiOutput');
    container.style.display = 'block';
    container.innerHTML = '<h3 style="color: var(--primary-d); margin-bottom: 20px;">📜 Kết quả luận giải</h3>';
    if (!results.length) {
      container.innerHTML += '<p style="color: var(--muted);">Không tìm thấy luận giải phù hợp.</p>';
      return;
    }
    results.forEach(r => {
      const div = document.createElement('div');
      div.className = 'luangiai-item';
      div.innerHTML = `
        <div class="lg-header">
          <span class="lg-type">${r.type}</span>
          <span class="lg-score">🎯 ${r.matchScore}% khớp</span>
        </div>
        <div class="lg-dieu-kien">Điều kiện: ${r.condition}</div>
        <div class="lg-loi-giai">${r.loi_giai}</div>
        ${r.tho ? `<div class="lg-tho">📖 ${r.tho}</div>` : ''}
      `;
  ...
  _extractLaSo() {
    ...
    return { cung, normalize: this._normalizeSao.bind(this), hasSao: (idx,n) => cung[idx]?.some(s => this._normalizeSao(s) === n) };
  }
  ...
  _matchRule(rule, laso, gocIdx) {
    if (!rule.stars.length) return null;
    if (rule.relation === 'dong_cung') {
      const expandedNorms = [];
      for (const rawStar of rule.stars) {
        const norm = this._normalizeSao(rawStar);
        const groups = { 'sat pha lang': ['that sat','pha quan','tham lang'], 'nhat nguyet': ['thai duong','thai am'] };
        if (groups[norm]) groups[norm].forEach(a => expandedNorms.push(a));
        else expandedNorms.push(norm);
      }
      for (let ci = 0; ci < 12; ci++) {
        if (!laso.cung[ci]) continue;
        if (expandedNorms.every(sn => laso.cung[ci].some(s => this._normalizeSao(s) === sn))) {
          let w;
          if (ci === gocIdx) w = 1.0;
          else if (this._tamHop(gocIdx).includes(ci)) w = 0.8;
          else if (ci === this._doiCung(gocIdx)) w = 0.7;
          else w = 0.5;
          return { type: rule.type, condition: rule.condition_raw, loi_giai: rule.loi_giai, tho: rule.tho, matchScore: Math.round(w * 100) };
        }
      }
      return null;
    }
    ...
    let minW = 1.0;
    for (const rawStar of rule.stars) {
      const w = this._timSao(rawStar, laso, gocIdx);
      if (w === 0) return null;
      minW = Math.min(minW, w);
    }
    if (rule.relation === 'tam_hop' && minW < 0.8) return null;
    return { type: rule.type, condition: rule.condition_raw, loi_giai: rule.loi_giai, tho: rule.tho, matchScore: Math.round(minW * 100) };
  }

  _timSao(starName, laso, gocIdx) {
    const norm = this._normalizeSao(starName);
    const groups = { 'sat pha lang': ['that sat','pha quan','tham lang'], 'nhat nguyet': ['thai duong','thai am'] };
    if (groups[norm]) {
      let minW = 1.0;
      for (const alias of groups[norm]) {
        const w = this._weightOfNorm(alias, laso, gocIdx);
        if (w === 0) return 0;
        minW = Math.min(minW, w);
      }
      return minW;
    }
    return this._weightOfNorm(norm, laso, gocIdx);
  }

  _weightOfNorm(normName, laso, gocIdx) {
    let maxW = 0;
    for (let i = 0; i < 12; i++) {
      if (!laso.cung[i]?.some(s => this._normalizeSao(s) === normName)) continue;
      let w;
      if (i === gocIdx) w = 1.0;
      else if (this._tamHop(gocIdx).includes(i)) w = 0.8;
      else if (i === this._doiCung(gocIdx)) w = 0.7;
      else if (i === this._nhiHop(gocIdx)) w = 0.3;
      else if (this._giapCung(gocIdx).includes(i)) w = 0.2;
      else w = 0.1;
  ...
  _tamHop(idx) { return [(idx + 4) % 12, (idx + 8) % 12]; }
  _doiCung(idx) { return (idx + 6) % 12; }
  _nhiHop(idx) { const map = {5:8,4:9,3:10,2:11,1:6,0:7,8:5,9:4,10:3,11:2,6:1,7:0}; return map[idx] ?? -1; }
  _giapCung(idx) { return [(idx + 11) % 12, (idx + 1) % 12]; }
}
